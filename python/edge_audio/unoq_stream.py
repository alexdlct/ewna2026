...
Sound Guardian service (python/): run `python deploy.py` to copy this runtime
and the model into python/edge_audio/. python/classifier.py then uses
StreamClassifier for CLASSIFIER=stream (audio over the USB relay) and
CLASSIFIER=model (microphone on the board).
"""
...
try:                                   # imported as a package: python/edge_audio/ (the UNO Q app)
    from . import config as C
    from .features import crop, load_audio, rms, strided_starts
    from .infer_wav import EventDecider, TFLiteClassifier
except ImportError:                    # run as plain modules from edge_audio_mvp/
    import config as C
    from features import crop, load_audio, rms, strided_starts
    from infer_wav import EventDecider, TFLiteClassifier
...
class StreamClassifier:
    ...
    def predict(self, window):
        """ModelClassifier.predict() contract: (label, confidence).

        Returns the Sound Guardian event id only for a CONFIRMED detection
        (threshold + agreement + cooldown, see EventDecider). Everything else
        comes back as "UNKNOWN". Raw class names must never leak out here:
        events.normalize_label() keyword-matches "LOUD_THUD" -> thud and would
        raise an alert on an unconfirmed window."""
        d, _, _ = self.step(window)
        if d.event and d.announce:
            return C.SOUND_GUARDIAN_EVENT.get(d.event) or d.event, d.confidence
        return "UNKNOWN", d.confidence
